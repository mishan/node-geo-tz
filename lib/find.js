...
function preCache () {
  ...
  var preloadFeaturesRecursive = function (curTzData, quadPos) {
    if (curTzData.pos && curTzData.len) {
      var geoJson = loadFeatures(quadPos, curTzData.pos, curTzData.len)
...
var loadFeatures = function (quadPos, pos, len) {
  if (featureFileFd < 0) {
    featureFileFd = fs.openSync(`${__dirname}/../data/geo.dat`, 'r')
  }
...
var getTimezone = function (originalLat, originalLon) {
  ...
  while (true) {
    ...
    } else if (curTzData.pos && curTzData.len) {
